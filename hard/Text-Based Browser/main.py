import sys
import os
import requests
from colorama import Fore
from bs4 import BeautifulSoup
nytimes_com = '''
This New Liquid Is Magnetic, and Mesmerizing

Scientists have created “soft” magnets that can flow 
and change shape, and that could be a boon to medicine 
and robotics. (Source: New York Times)


Most Wikipedia Profiles Are of Men. This Scientist Is Changing That.

Jessica Wade has added nearly 700 Wikipedia biographies for
 important female and minority scientists in less than two 
 years.

'''

bloomberg_com = '''
The Space Race: From Apollo 11 to Elon Musk

It's 50 years since the world was gripped by historic images
 of Apollo 11, and Neil Armstrong -- the first man to walk 
 on the moon. It was the height of the Cold War, and the charts
 were filled with David Bowie's Space Oddity, and Creedence's 
 Bad Moon Rising. The world is a very different place than 
 it was 5 decades ago. But how has the space race changed since
 the summer of '69? (Source: Bloomberg)


Twitter CEO Jack Dorsey Gives Talk at Apple Headquarters

Twitter and Square Chief Executive Officer Jack Dorsey 
 addressed Apple Inc. employees at the iPhone maker’s headquarters
 Tuesday, a signal of the strong ties between the Silicon Valley giants.
'''
dir = sys.argv[1]
stack = []
if not os.path.exists(dir):
    os.mkdir(dir)
while True:
    site = input()

    if site == "back":
        if (len(stack) > 1):
            stack.pop()
            print(stack.pop())
    elif site == "exit":
        break
    elif site.find('.') == -1:
        if not os.path.isfile(dir+'/'+site):
            print('Error: Incorrect URL')
        else:
            with open(f"{dir}/{site}",'r') as f:
                line = f.read()
                stack.append(line)
                print(f.read())
    elif not site.startswith("https://"):
        site = "https://" + site
        r = requests.get(site)
        soup = BeautifulSoup(r.content,'html.parser')
        tags = soup.find_all(['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'a', 'ul', 'ol', 'li'])
        text = ""
        for p in tags:
            if p.name == 'a':
                text += Fore.BLUE + p.text + "\n"
            else:
                text += p.text + "\n"
        print(text)
        with open(f"{dir}/{site[8:site.find('.')]}", 'w') as f:
            f.write(text)
        stack.append(text)
    #elif site == 'bloomberg.com':
    #    with open(f"{dir}/{site[:site.find('.')]}",'w') as f:
    #        f.write(bloomberg_com)
    #    stack.append(bloomberg_com)
    #    print(bloomberg_com)
    #elif site == 'nytimes.com':
    #    with open(f"{dir}/{site[:site.find('.')]}",'w') as f:
    #        f.write(nytimes_com)
    #    stack.append(nytimes_com)
    #    print(nytimes_com)
    else:
        print('Error')

